...
- **`/index.html`** — site root; hands the URL to the style guide
- **`/styleguide/`** — the style guide, built by Jekyll from `products/styleguide/` (as of the Pages Actions migration; v3.3.0–v3.5.x mirrored `axismundi-lab/style-guide*.html`, and before that `axismundi-prototype/`)

GitHub Pages is built and deployed by `.github/workflows/styleguide.yml` from an
Actions artifact. Only the two surfaces above are published. Until the migration
Pages served every tracked file from `main`, which put `corpus/`, `atlas/`,
`docs/` and the lab's pattern HTML online as a consequence of the build type
rather than as a decision; those are readable in the repository and locally.
...
4. **Publishing surfaces are not layers.** Removing `/styleguide/` doesn't lose information — the lab still has its own style guide, and `products/styleguide/` is the source of the published one. Removing either **does** lose information.
   The lab's mirror was deleted when Pages moved to Actions: it had become a second answer to a URL the documentation product now owns. The lab itself is untouched and opens from the file system.
...
- **Thread context is a per-topic resolvable URI**, not the Group URI. FEP-7888 explicitly permits a forum, channel, or group as `context`, so `context = Group` is a valid reading of that FEP and was not a defect. The reason to move is FEP-11dd: inheritance gives a reply its parent's context, and if that context is the Group then every reply in the Forum inherits the same value and no thread can be named. A per-topic context is what the inheritance rule needs something to be *about*.
