...
- **v3.6.0 (NEXT)** — **Ontology Theme Pilot**. Theme-only WordPress block theme proof consuming Wave 1 minus Carousel + infrastructure dependencies (popover/, ripple/, icon-system/) plus blocks/prose Pilot specification surfaces. Carousel remains plugin territory (BACKLOG #38). Validates 4-tier architecture in production. Per CHARTER §3.4, pilot lives outside the distributable theme repo; the theme repo provides the public surface only.
