...
- **v3.6.19** ← **Asset Surface Audit + Cross-Reference Index** ✓ DONE (2026-05-24)
  - Closed a narrow documentation-hygiene asset surface audit without path consolidation, asset movement, runtime reference changes, or publish mirror regeneration.
  - Added `docs/ASSET-SURFACE-INDEX.md` as the cross-cutting asset authority index and recorded seven surfaces: root brand/media, brand research, M3 core assets, Pilot assets, ontology-theme-pilot assets, and styleguide mirror.
...
- **v3.6.20** ← **Pilot vs Distributable Bootstrap** ✓ DONE (2026-05-24)
  ...
  - Verified `php -l`, `npm test` Axis A-G all 1.000, `build_pilot_specimen_wall`, `validate:specimen-wall`, `validate:computed`, and `git diff --check`; validator-generated report churn was restored.
- **v3.6.21** ← **Theme Switcher Contract** ✓ DONE (2026-05-24)
  - Closed an A-only no-code theme-switcher contract decision: `.sg-theme` is the lab/styleguide/module selector, `.ax-theme-switcher` is the Pilot/future product-facing selector, and current runtime tolerance means there is no click-path defect.
  - Formalized `data-theme-button` as styleguide-local, `data-theme-set` as production/module/Pilot runtime vocabulary, and owner-specific storage keys instead of a globally unified visitor key.
  - Narrowed BACKLOG #22 to explicit `data-theme="auto"` root-state implementation and preserved BACKLOG #21 as plugin territory for HCT, editor UI, Global Styles sync, and custom color regeneration.
  - Verified `php -l`, `npm test` Axis A-G all 1.000, `build_pilot_specimen_wall`, `validate:specimen-wall`, `validate:computed`, and `git diff --check`; validator-generated report churn was restored.
- **v3.6.22** ← **Explicit `data-theme="auto"` Root State** ✓ DONE (2026-05-24)
  - Closed BACKLOG #22 as a narrow implementation cycle across lab, generated styleguide, and Pilot surfaces.
  - JS now writes explicit `data-theme="auto"` for auto mode; CSS selectors distinguish absent root theme from explicit auto; Pilot gets a front-end-only `language_attributes` root default guarded away from admin/editor contexts.
  - Phase 2 amend restored the v3.6.17 Pilot bridge source/copy byte-identical contract.
  - Verified full validation, six JS syntax checks, browser/runtime paths, bridge source/copy SHA256, generated artifact restore, and `git diff --check`.
- **v3.6.23** ← **Core Block Catalog 6-Category Split** ✓ DONE (2026-05-24)
  - Executed the v3.6.18 routed-forward catalog split as a narrow implementation cycle.
  - Reorganized `style-guide-blocks.html` and generated `styleguide/blocks.html` around Text, Media, Design, Widgets, and Theme, with Embeds excluded.
  - Preserved validator anchor `#blocks-table`; added `#blocks-search` and `#blocks-theme`.
  - Verified publish tooling, computed/specimen validation, browser smoke, generated churn restoration, and `git diff --check`.
- **v3.6.24** ← **Core Block Style Guide Full Spec** ✓ DONE (2026-05-24)
  - Completed the catalog full-spec pass after the v3.6.23 shell.
  - Added Heading, Cover, and Media & Text specimens plus explicit gap rows across Text, Media, Design, Widgets, and Theme/FSE.
  - Preserved `#blocks-table`, `#blocks-search`, and `#blocks-theme`; added `#blocks-heading`, `#blocks-text-gaps`, and `#blocks-design-gaps`.
  - Formalized the catalog cycle template: 4-state classification, M9 source/validator inventory first, Route D + M3 publish tooling, local HTTP browser smoke, and 7-file single close commit.
  - Verified publish tooling, computed/specimen validation, `npm test` Axis A-G all 1.000, local HTTP browser smoke, generated churn restore, optional untracked artifact cleanup, and `git diff --check`.
- **v3.6.25** ← **Webdesign Decision Matrix Ontology** ✓ DONE (2026-05-25)
  - Promoted webdesign methodology notes into refined corpus, web-production workflow atlas, and core web-production ontology.
  - Established the five-tier documentation retention policy: keep / archive / fold / restore_remove / route_forward.
  - Kept `decisions/` route-forward, avoided a broad `knowledge/` umbrella, and routed TT5 / Google Sites / Pilot implementation as later matrix consumers.
- **v3.6.26** ← **TT5 Docs + Codebase Audit Phase 0** ✓ DONE (2026-05-25)
  - Added the TT5 Phase 0 plan with official WordPress.org documentation as behavior evidence and local TT5 1.5 files as schema/template evidence.
  - Confirmed TT5 local inventory: templates, parts, patterns, style variations, block styles, section styles, typography presets, and color presets.
  - Locked TT5 as comparator evidence, not Axismundi visual authority.
- **v3.6.27** ← **Pilot TT5 Structural Transplant** ✓ DONE (2026-05-25)
  - Transplanted TT5 structural evidence into Axismundi Pilot: WP 6.7 theme.json schema, 645px / 1340px layout, 8-template surface, pattern-driven header, and additive JSON block/section style variations.
  - Preserved Pilot MD3 token ownership: no TT5 hex values, Manrope / Fira Code, TT5 pattern content, `twentytwentyfive` namespace, or `accent-*` imports.
  - Verified `npm test` Axis A-G all 1.000, computed/specimen gates, wp-env WordPress 7.0 active theme, `WP_Theme_JSON_Resolver::get_style_variations("block")` returning all 8 JSON variations, front-end route smoke, and `git diff --check`.
- **v3.6.28+ (NEXT)** — Post-TT5 Pilot/application route. Candidate next work: Google Sites extraction using the v3.6.25 matrix and v3.6.27 Pilot structure; or a narrow Pilot hygiene cycle for inverse palette slugs / `.wp-env.json` runtime-version staleness / optional footer pattern conversion. Distributable skeleton remains blocked until explicit user slug / product GO. Other routed follow-ons remain Theme Switcher Route B comment hygiene, release-seal derivative generation, distributable build-copy pipeline, Media catalog implementation, Pixabay video isolation, `ontology-theme-pilot/assets/` modernization/freeze, BACKLOG #21 Interpreter Plugin strategy, remaining BACKLOG #44 specimen coverage follow-ons, BACKLOG #46 disabled ripple host hygiene, BACKLOG #47 popover provider menu-item-class logic extraction hygiene, Sheet drag-to-dismiss follow-on, styleguide integration for Slider/Loading/Progress, VS Code workspace diagnostics config policy, Microsoft Edge Tools / webhint policy decisions, no-inline-styles policy, broad compat-api/css policy, or button-group `inline-size: fit-content` compatibility.
