---
source_url: https://developer.wordpress.org/block-editor/how-to-guides/curating-the-editor-experience/disable-editor-functionality/
synced: 2026-05-12
...
There might be times when you don’t want access to a block at all to be available for users. To control what’s available in the inserter, you can take two approaches: [an allow list](../../03-reference-guides/03-hooks-reference/block-filters.md#using-an-allow-list) that disables all blocks except those on the list or a [deny list that unregisters specific blocks](../../03-reference-guides/03-hooks-reference/block-filters.md#using-a-deny-list).
...
You can also use [block filters](../../03-reference-guides/03-hooks-reference/block-filters.md) to set the default value of this attribute globally or for specific blocks. The example below disables H1, H2, and H6 for all Heading blocks. You can further customize this by restricting certain heading levels based on conditions like user capabilities.
...
Some Core blocks are actually [block variations](../../03-reference-guides/01-block-api-reference/variations.md). A great example is the Row and Stack blocks, which are actually variations of the Group block. If you want to disable these “blocks”, you actually need to disable the respective variations.
...
This JavaScript should be enqueued much like the block variation example above. Refer to the [block styles](../../03-reference-guides/01-block-api-reference/styles.md) documentation for how to register and unregister styles using PHP.
...
```php
function example_restrict_code_editor_access( $settings, $context ) { $settings[ 'codeEditingEnabled' ] = false; return $settings;}add_filter( 'block_editor_settings_all', 'example_restrict_code_editor_access', 10, 2 );
```
...
Blocks that support [RichText](../../03-reference-guides/06-richtext-reference/index.md) come with the default formatting options provided by WordPress.
