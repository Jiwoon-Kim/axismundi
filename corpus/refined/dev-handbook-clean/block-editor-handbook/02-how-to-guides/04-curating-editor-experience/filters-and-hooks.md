---
source_url: https://developer.wordpress.org/block-editor/how-to-guides/curating-the-editor-experience/filters-and-hooks/
synced: 2026-05-12
...
- `$settings` – An array of [configurable settings](../../03-reference-guides/03-hooks-reference/editor-hooks.md#editor-settings) for the Editor.
...
```php
add_filter( 'block_editor_settings_all', 'example_restrict_code_editor' ); function example_restrict_code_editor( $settings ) { $can_active_plugins = current_user_can( 'activate_plugins' ); // Disable the Code Editor for users that cannot activate plugins (Administrators). if ( ! $can_active_plugins ) { $settings[ 'codeEditingEnabled' ] = false; } return $settings;}
```

For more examples, check out the [Editor Hooks](../../03-reference-guides/03-hooks-reference/editor-hooks.md) documentation that includes the following use cases:

- [Set a default image size](../../03-reference-guides/03-hooks-reference/editor-hooks.md#set-a-default-image-size)
- [Disable Openverse](../../03-reference-guides/03-hooks-reference/editor-hooks.md#disable-openverse)
- [Disable the Font Library](../../03-reference-guides/03-hooks-reference/editor-hooks.md#disable-the-font-library)
- [Disable block inspector tabs](../../03-reference-guides/03-hooks-reference/editor-hooks.md#disable-block-inspector-tabs)
...
WordPress 6.2 introduced a new client-side filter allowing you to modify block-level [theme.json settings](../../03-reference-guides/07-theme-json-reference/version-3-reference.md#settings) before the Editor is rendered.
...
```js
import { addFilter } from '@wordpress/hooks'; /** * Limit the Column block's spacing options to pixels. */addFilter( 'blockEditor.useSetting.before', 'example/useSetting.before', ( settingValue, settingName, clientId, blockName ) => { if ( blockName === 'core/column' && settingName === 'spacing.units' ) { return [ 'px' ]; } return settingValue; });
```
...
```js
import { select } from '@wordpress/data';import { addFilter } from '@wordpress/hooks'; /** * Disable text color controls on Heading blocks when placed inside of Media & Text blocks. */addFilter( 'blockEditor.useSetting.before', 'example/useSetting.before', ( settingValue, settingName, clientId, blockName ) => { if ( blockName === 'core/heading' ) { const { getBlockParents, getBlockName } = select( 'core/block-editor' ); const blockParents = getBlockParents( clientId, true ); const inMediaText = blockParents.some( ( ancestorId ) => getBlockName( ancestorId ) === 'core/media-text' ); if ( inMediaText && settingName === 'color.text' ) { return false; } } return settingValue; });
```
...
The `$metadata` array contains everything you might want to know about a block, from its description and [attributes](../../03-reference-guides/01-block-api-reference/attributes.md) to block [supports](../../03-reference-guides/01-block-api-reference/supports.md).
...
You can learn more about the available block filters in the [Block Filters](../../03-reference-guides/03-hooks-reference/block-filters.md) documentation.
