---
source_url: https://developer.wordpress.org/block-editor/how-to-guides/metabox/
synced: 2026-05-12
...
The block editor does support most existing meta boxes, see [the backward compatibility section below](meta-boxes.md#backward-compatibility) for details .

If you are interested in working with the post meta outside the editor, check out the [Sidebar Tutorial](https://developer.wordpress.org/block-editor/how-to-guides/sidebar-tutorial/plugin-sidebar-0/).
...
This guide assumes you are already familiar with WordPress plugins, post meta, and basic JavaScript. Review the [Getting started with JavaScript tutorial](../01-getting-started/02-fundamentals-of-block-development/javascript-in-the-block-editor.md) for an introduction.

The guide will walk through creating a basic block, but recommended to go through the [Create Block tutorial](https://developer.wordpress.org/block-editor/getting-started/tutorial/) for a deeper understanding of creating custom blocks.
...
1. [Register Meta Field](meta-boxes.md#step-1-register-meta-field)
2. [Add Meta Block](meta-boxes.md#step-2-add-meta-block)
3. [Use Post Meta Data](meta-boxes.md#step-3-use-post-meta-data)
4. [Finishing Touches](meta-boxes.md#step-4-use-block-templates-optional)

### Step 1: Register meta field

A post meta field is a WordPress object used to store extra data about a post. You need to first register a new meta field prior to use. See Managing [Post Metadata](../../plugin-handbook/09-metadata/managing-post-metadata.md) to learn more about post meta.
...
```php
<?php// register custom meta tag fieldfunction myguten_register_post_meta() { register_post_meta( 'post', 'myguten_meta_block_field', array( 'show_in_rest' => true, 'single' => true, 'type' => 'string', ) );}add_action( 'init', 'myguten_register_post_meta' );
```
...
```php
function myguten_content_filter( $content ) { $value = get_post_meta( get_the_ID(), 'myguten_meta_block_field', true ); if ( $value ) { return sprintf( "%s <h4> %s </h4>", $content, esc_html( $value ) ); } else { return $content; }}add_filter( 'the_content', 'myguten_content_filter' );
```
...
```php
function myguten_render_paragraph( $block_attributes, $content ) { $value = get_post_meta( get_the_ID(), 'myguten_meta_block_field', true ); // check value is set before outputting if ( $value ) { return sprintf( "%s (%s)", $content, esc_html( $value ) ); } else { return $content; }} register_block_type( 'core/paragraph', array( 'api_version' => 3, 'render_callback' => 'myguten_render_paragraph',) );
```

### Step 4: Use block templates (optional)

One problem using a meta block is the block is easy for an author to forget, since it requires being added to each post. You solve this by using [block templates](../03-reference-guides/01-block-api-reference/templates.md). A block template is a predefined list of block items per post type. Templates allow you to specify a default initial state for a post type.
