...
The Interactivity API is supported by the package [`@wordpress/interactivity`](../09-package-reference/packages/interactivity.md) which is bundled in WordPress Core from v6.5
...
- **[Requirements](index.md#requirements-of-the-interactivity-api):** Check this section before you start creating your interactive blocks with the Interactivity API.
- **[Quick Start Guide](quick-start-guide.md):** Get a custom block using the Interactivity API up and running in less than one minute.
...
- **[Core Concepts](01-core-concepts/index.md):** Gain a better understanding of concepts and mental models related to Interactivity API development from this section.
- **[Directives and Store](directives-and-store.md):** To take a deep dive into how the API works internally, the list of Directives, and how the Store works.
- **[Docs and Examples](index.md#docs--examples):** Additional resources to learn/read more about the Interactivity API.
...
- **[About the Interactivity API](about-interactivity-api.md):** To learn more about the API Goals and the reasoning behind the use of a standard to add interactivity to blocks.
- **[Frequently Asked Questions](faq.md):** To find responses to some frequently asked questions about the technology behind and alternatives.
...
Import the store into your `view.js`. Refer to the [store documentation](directives-and-store.md#the-store) for more information.
...
To indicate that the block [supports](../01-block-api-reference/supports.md) the Interactivity API features, add `"interactivity": true` to the `supports` attribute of the block’s `block.json` file.
...
Refer to the [`interactivity` support property docs](../01-block-api-reference/supports.md#interactivity) to get a more detailed description of this property.

#### Load Interactivity API JavaScript code with viewScriptModule

The Interactivity API provides the `@wordpress/interactivity` Script Module. JavaScript using the Interactivity API should be implemented as Script Modules so they can depend on `@wordpress/interactivity`. [Script Modules have been available since WordPress 6.5](https://make.wordpress.org/core/2024/03/04/script-modules-in-6-5/). Blocks can use [`viewScriptModule` block metadata](../01-block-api-reference/metadata-in-block-json.md#view-script-module) to enqueue their Script Modules easily:
...
The use of `viewScriptModule` also requires the `--experimental-modules` flag for both the [`build`](../09-package-reference/packages/scripts.md#build) and [`start`](../09-package-reference/packages/scripts.md#start) scripts of `wp-scripts` to ensure a proper build of the Script Modules.
...
```json
{ "scripts": { "build": "wp-scripts build --experimental-modules", "start": "wp-scripts start --experimental-modules" }}
```
...
```html
<div data-wp-interactive="myPlugin"> <!-- Interactivity API zone --></div>
```

Refer to the [`wp-interactive` documentation](directives-and-store.md#wp-interactive) for a more detailed description of this directive.
