---
source_url: https://developer.wordpress.org/block-editor/reference-guides/components/combobox-control/
synced: 2026-05-12
...
```jsx
import { useState } from 'react';import { ComboboxControl } from '@wordpress/components'; const options = [ { value: 'small', label: 'Small', }, { value: 'normal', label: 'Normal', }, { value: 'large', label: 'Large', },]; function MyComboboxControl() { const [ fontSize, setFontSize ] = useState(); const [ filteredOptions, setFilteredOptions ] = useState( options ); return ( <ComboboxControl __next40pxDefaultSize label="Font Size" value={ fontSize } onChange={ setFontSize } isLoading={ isLoading } options={ filteredOptions } onFilterValueChange={ ( inputValue ) => setFilteredOptions( options.filter( ( option ) => option.value === inputValue ) ) } /> );}
```
...
- Like this component, but without a search input, the [`CustomSelectControl`](custom-select-control.md) component.
- To select one option from a set, when you want to show all the available options at once, use the [`RadioControl`](radio-control.md) component.
- To select one or more items from a set, use the [`CheckboxControl`](checkbox-control.md) component.
- To toggle a single setting on or off, use the [`ToggleControl`](toggle-control.md) component.
