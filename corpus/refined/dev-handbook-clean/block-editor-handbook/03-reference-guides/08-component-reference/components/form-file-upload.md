---
source_url: https://developer.wordpress.org/block-editor/reference-guides/components/form-file-upload/
synced: 2026-05-12
...
```jsx
import { FormFileUpload } from '@wordpress/components'; const MyFormFileUpload = () => ( <FormFileUpload __next40pxDefaultSize accept="image/*" onChange={ ( event ) => console.log( event.currentTarget.files ) } > Upload </FormFileUpload>);
```
...
```js
<FormFileUpload __next40pxDefaultSize onClick={ ( event ) => ( event.target.value = '' ) } onChange={ onChange }> Upload</FormFileUpload>
```
