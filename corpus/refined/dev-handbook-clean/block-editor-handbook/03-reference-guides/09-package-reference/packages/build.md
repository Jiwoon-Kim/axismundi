---
source_url: https://developer.wordpress.org/block-editor/reference-guides/packages/packages-wp-build/
synced: 2026-05-12
...
For more details on when to omit or set this to `false`, see the [package guidelines](https://developer.wordpress.org/block-editor/reference-guides/packages/README.md#when-to-omit-or-set-wpscript-to-false).
...
```json
{ "wpScriptModuleExports": { "./interactivity": "./build-module/interactivity/index.js" }}
```
...
```json
{ "wpStyleEntryPoints": { "style": "src/style.scss" }}
```
...
```json
{ "wpCopyFiles": [ { "from": "src/index.php", "to": "build/index.php", "transform": "php" } ]}
```
...
```json
{ "wpWorkers": { "./worker": "./src/worker.ts" }}
```

**Object format** — entry path with module resolve redirects:

```json
{ "wpWorkers": { "./worker": { "entry": "./src/worker.ts", "resolve": { "vips-es6.js": "vips.js" } } }}
```
...
```json
{ "wpPlugin": { "scriptGlobal": "myPlugin" }}
```
...
```json
{ "wpPlugin": { "scriptGlobal": "myPlugin", "packageNamespace": "my-plugin" }}
```
...
```json
{ "wpPlugin": { "scriptGlobal": "myPlugin", "packageNamespace": "my-plugin", "handlePrefix": "mp" }}
```
...
```json
{ "wpPlugin": { "externalNamespaces": { "woo": { "global": "woo", "handlePrefix": "woocommerce" }, "acme": { "global": "acme", "handlePrefix": "acme-plugin" } } }}
```
...
```json
{ "wpPlugin": { "pages": [ "my-admin-page", { "id": "my-other-page", "init": ["@my-plugin/my-page-init"] } ] }}
```
...
```php
add_submenu_page( 'themes.php', // Parent menu __( 'My Page', 'my-plugin' ), // Page title __( 'My Page', 'my-plugin' ), // Menu title 'edit_theme_options', // Capability 'my-admin-page-wp-admin', // Menu slug (simple) 'my_plugin_my_admin_page_wp_admin_render_page' // Callback from generated PHP (prefixed));
```
...
```php
// Link to a specific route$url = admin_url( 'admin.php?page=my-admin-page-wp-admin&p=' . urlencode( '/settings' ) );
```
...
```json
{ "name": "@my-plugin/my-page-init", "wpScriptModuleExports": "./build-module/index.js", "dependencies": { "@wordpress/boot": "file:../boot", "@wordpress/data": "file:../data", "@wordpress/icons": "file:../icons" }}
```
...
```js
import { home, styles } from '@wordpress/icons';import { dispatch } from '@wordpress/data';import { store as bootStore } from '@wordpress/boot'; /** * Initialize page - this function is mandatory. * All init modules must export an 'init' function. */export async function init() { // Add icons to menu items dispatch( bootStore ).updateMenuItem( 'home', { icon: home } ); dispatch( bootStore ).updateMenuItem( 'styles', { icon: styles } );}
```
...
```json
{ "wpPlugin": { "scriptGlobal": "wp", "packageNamespace": "wordpress" }}
```
...
```json
{ "wpPlugin": { "name": "acme", "scriptGlobal": "acme", "packageNamespace": "acme" }}
```
...
```text
routes/ home/ package.json # Route configuration stage.tsx # Main content component inspector.tsx # Optional sidebar component canvas.tsx # Optional custom canvas component route.tsx # Optional lifecycle hooks (beforeLoad, loader, canvas)
```
...
```json
{ "route": { "path": "/", "page": "my-admin-page" }}
```

For routes that should appear on multiple pages:

```json
{ "route": { "path": "/settings", "page": ["my-admin-page", "other-page"] }}
```
...
```php
export const route = { beforeLoad: ({ params, search }) => { // Pre-navigation validation, auth checks }, loader: ({ params, search }) => { // Data preloading }, canvas: ({ params, search }) => { // Return CanvasData to use default canvas (editor) return { postType: 'post', postId: '123', isPreview: true }; // Return null to use custom canvas.tsx component // return null; // Return undefined to show no canvas // return undefined; }};
```
...
```text
widgets/ hello-world/ widget.json # Static discovery metadata (required) widget.ts # Runtime schema entry point (optional) render.tsx # UI component entry point (optional) render.scss # Optional styles (bundled inline when imported from render.tsx)
```
...
```json
{ "name": "my-plugin/hello-world", "title": "Hello World", "description": "A simple example widget.", "category": "demo"}
```
...
```js
import { __ } from '@wordpress/i18n'; type HelloWorldAttributes = { greeting: string; showDate: boolean;}; const widget = { name: 'my-plugin/hello-world', title: __( 'Hello World', 'my-plugin' ), attributes: [ { id: 'greeting', type: 'text', label: __( 'Greeting', 'my-plugin' ), }, { id: 'showDate', type: 'boolean', label: __( 'Show current date', 'my-plugin' ), }, ], example: { greeting: 'World', showDate: true, },}; export default widget;
```
...
```js
interface HelloWorldRenderProps { attributes: { greeting?: string; showDate?: boolean; };} export default function HelloWorld( { attributes }: HelloWorldRenderProps ) { return ( <div> Hello, { attributes.greeting ?? 'World' }! { attributes.showDate && <time>{ new Date().toLocaleDateString() }</time> } </div> );}
```
...
```text
{handlePrefix}/widgets/{widget-dir-name}/render{handlePrefix}/widgets/{widget-dir-name}/widget
```
