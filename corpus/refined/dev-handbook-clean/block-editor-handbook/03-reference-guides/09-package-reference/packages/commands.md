---
source_url: https://developer.wordpress.org/block-editor/reference-guides/packages/packages-commands/
synced: 2026-05-12
...
For example, when a user types “contact”, the Command Palette needs to filter the available pages using that input to try and find the Contact page. See the `useCommandLoader` [code example](commands.md#usecommandloader) below.
...
The Command Palette also offers a number of [selectors and actions](../../10-data-module-reference/data-core-commands.md) to manipulate its state, which include:
...
See the [Commands Data](../../10-data-module-reference/data-core-commands.md) documentation for more information.
...
```js
/* From node_modules: */@import '@wordpress/components/build-style/style.css';@import '@wordpress/commands/build-style/style.css';
```
...
```js
import { store as commandsStore } from '@wordpress/commands';import { useDispatch } from '@wordpress/data';...const { open: openCommandCenter } = useDispatch( commandsStore );
```
...
```js
import { useCommand } from '@wordpress/commands';import { plus } from '@wordpress/icons'; useCommand( { name: 'myplugin/my-command-name', label: __( 'Add new post' ), icon: plus, category: 'command', callback: ( { close } ) => { document.location.href = 'post-new.php'; close(); },} );
```
