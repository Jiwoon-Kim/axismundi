---
source_url: https://developer.wordpress.org/block-editor/reference-guides/packages/packages-interactivity-router/
synced: 2026-05-12
...
For a comprehensive guide on how client-side navigation works, including getting started, block compatibility, and advanced use cases, see the [Client-Side Navigation guide](../../04-interactivity-api-reference/01-core-concepts/client-side-navigation.md). To learn how to ensure your blocks are compatible with client-side navigation, see the [Client-Side Navigation Compatibility guide](../../04-interactivity-api-reference/01-core-concepts/navigation-compatibility.md).
...
```js
/* view.js */ import { store, withSyncEvent } from '@wordpress/interactivity'; // This is how you would typically use the navigate() action in your block.store( 'my-namespace/myblock', { actions: { // The withSyncEvent() utility needs to be used because preventDefault() requires synchronous event access. goToPage: withSyncEvent( function* ( e ) { e.preventDefault(); // We import the package dynamically to reduce the initial JS bundle size. // Async actions are defined as generators so the import() must be called with `yield`. const { actions } = yield import( '@wordpress/interactivity-router' ); yield actions.navigate( e.target.href ); } ), },} );
```
...
```js
const { state, actions } = store( 'core/router', { state: { url: window.location.href, }, actions: { *navigate(href, options) {...}, prefetch(url, options) {...}, }})
```
...
```html
<div data-wp-interactive="myblock" data-wp-router-region="main-list"> <ul> <li><a href="/post-1">Post 1</a></li> <li><a href="/post-2">Post 2</a></li> <li><a href="/post-3">Post 3</a></li> </ul> <a data-wp-on--click="actions.navigate" href="/page/2">Page 2</a></div>
```
...
```html
<div data-wp-interactive="myblock" data-wp-router-region='{ "id": "myblock/overlay", "attachTo": "body" }'> I'm in a new region!</div>
```
