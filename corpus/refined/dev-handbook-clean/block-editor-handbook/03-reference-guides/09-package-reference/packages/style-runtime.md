---
source_url: https://developer.wordpress.org/block-editor/reference-guides/packages/packages-style-runtime/
synced: 2026-05-12
...
```js
import { registerStyle } from '@wordpress/style-runtime'; registerStyle( 'style-hash', '.example{color:red;}' );
```
...
```js
import { registerDocument } from '@wordpress/style-runtime'; const unregisterDocument = registerDocument( iframe.contentDocument ); // Later, when the document is no longer used:unregisterDocument();
```
...
```js
import { registerDocument } from '@wordpress/style-runtime'; const cleanup = registerDocument( document ); cleanup();
```
