---
source_url: https://developer.wordpress.org/plugins/hooks/custom-hooks/
synced: 2026-05-12
handbook: plugin
chapter: hooks
slug: custom-hooks
...
To create a custom hook, use `do_action()` for [Actions](actions.md) and `apply_filters()` for [Filters](filters.md).
...
To add a callback function to a custom hook, use `add_action()` for [Actions](actions.md) and `add_filter()` for [Filters](filters.md).
...
```php
function wporg_create_post_type() { $post_type_params = [/* ... */]; register_post_type( 'post_type_slug', apply_filters( 'wporg_post_type_params', $post_type_params ) );}
```
...
```php
function myprefix_change_post_type_params( $post_type_params ) { $post_type_params['hierarchical'] = true; return $post_type_params;}add_filter( 'wporg_post_type_params', 'myprefix_change_post_type_params' );
```
