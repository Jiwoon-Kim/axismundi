---
source_url: https://developer.wordpress.org/plugins/hooks/filters/
synced: 2026-05-12
handbook: plugin
chapter: hooks
slug: filters
...
**Filters** are one of the two types of [Hooks](index.md).

They provide a way for functions to modify data during the execution of WordPress Core, plugins, and themes. They are the counterpart to [Actions](actions.md).

Unlike [Actions](actions.md), filters are meant to work in an isolated manner, and should never have [side effects](https://en.wikipedia.org/wiki/Side_effect_%28computer_science%29) such as affecting global variables and output. Filters expect to have something returned back to them.
...
```php
function wporg_filter_title( $title ) { return 'The ' . $title . ' was filtered';}add_filter( 'the_title', 'wporg_filter_title' );
```
...
You can refer to the [Hooks](index.md) chapter for a list of available hooks.
...
For detailed explanation of these parameters please read the article on [Actions](actions.md).
...
```php
function wporg_css_body_class( $classes ) { if ( ! is_admin() ) { $classes[] = 'wporg-is-awesome'; } return $classes;}add_filter( 'body_class', 'wporg_css_body_class' );
```
