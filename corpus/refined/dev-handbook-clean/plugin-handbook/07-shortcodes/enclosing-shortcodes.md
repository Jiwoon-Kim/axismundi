---
source_url: https://developer.wordpress.org/plugins/shortcodes/enclosing-shortcodes/
synced: 2026-05-12
handbook: plugin
chapter: shortcodes
...
- The shortcode is a self-closing tag like we seen in the [Basic Shortcodes](basic-shortcodes.md) section.
...
```php
function wporg_shortcode( $atts = array(), $content = null ) { // do something to $content // always return return $content;}add_shortcode( 'wporg', 'wporg_shortcode' );
```
...
```php
function wporg_shortcode( $atts = array(), $content = null ) { // do something to $content // run shortcode parser recursively $content = do_shortcode( $content ); // always return return $content;}add_shortcode( 'wporg', 'wporg_shortcode' );
```
