...
parent_order: 16
page_order: 1
title: "Scheduling WP Cron Events"
...
Remember, the “bl\_” part of the function name is a *function prefix*. You can learn why prefixes are important [here](../02-plugin-basics/best-practices.md#prefix-everything).

You can read more about actions [here](../04-hooks/actions.md).
...
We will use the 5 second interval we created [here](understanding-wp-cron-scheduling.md) and the hook we created above, like so:
...
```php
if ( ! wp_next_scheduled( 'bl_cron_hook' ) ) { wp_schedule_event( time(), 'five_seconds', 'bl_cron_hook' );}
```
...
```php
$timestamp = wp_next_scheduled( 'bl_cron_hook' );wp_unschedule_event( $timestamp, 'bl_cron_hook' );
```
...
You can read more about activation and deactivation hooks [here](../02-plugin-basics/activation-deactivation-hooks.md).
