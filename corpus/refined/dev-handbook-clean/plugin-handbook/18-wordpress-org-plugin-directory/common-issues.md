---
source_url: https://developer.wordpress.org/plugins/wordpress-org/common-issues/
synced: 2026-05-12
handbook: plugin
...
[https://developer.wordpress.org/apis/security/sanitizing/](../../common-apis-handbook/16-security/sanitizing-data.md) [https://developer.wordpress.org/apis/security/escaping/](../../common-apis-handbook/16-security/escaping-data.md)

Remember: You must use the most appropriate functions for the context. If you’re sanitizing email, use [sanitize_email()](https://developer.wordpress.org/reference/functions/sanitize_email/) , if you’re outputting HTML, use [wp_kses_post()](https://developer.wordpress.org/reference/functions/wp_kses_post/) , and so on.
...
```php
<h2><?php echo esc_html__('Settings page', 'plugin-slug'); ?></h2><h2><?php echo esc_html(__('Settings page', 'plugin-slug')); ?></h2>
```
...
```php
<h2><?php esc_html_e('Settings page', 'plugin-slug'); ?></h2><h2><?php echo esc_html(__('Settings page', 'plugin-slug')); ?></h2><h2><?php echo esc_html(_x('Settings page', 'Settings page title', 'plugin-slug')); ?></h2>
```
...
```php
echo wp_kses_post($html_content);echo wp_kses($html_content, array( 'a', 'div', 'span' ));
```
...
```php
$wordcamp_id_placeholders = implode( ', ', array_fill( 0, count( $wordcamp_ids ), '%d' ) );$prepare_values = array_merge( array( $new_status ), $wordcamp_ids );$wpdb->query( $wpdb->prepare( " UPDATE `$table_name` SET `post_status` = %s WHERE ID IN ( $wordcamp_id_placeholders )", $prepare_values) );
```
...
Also WordPress explicitly sets and expects the default timezone to be UTC (in settings.php) and the date/time functions sometimes rely on the fact that the default timezone is UTC. For instance if you do date\_default\_timezone\_set(get\_option(‘timezone\_string’)) and then later try to get a GMT timestamp from [get_post_time()](https://developer.wordpress.org/reference/functions/get_post_time/)  or [get_post_modified_time()](https://developer.wordpress.org/reference/functions/get_post_modified_time/) , it will fail to give you the right date.
...
Please check out our tips on how to [structure files and folders in a plugin](../02-plugin-basics/best-practices.md#folder-structure).
...
- [wp_register_script()](https://developer.wordpress.org/reference/functions/wp_register_script/) and [wp_enqueue_script()](https://developer.wordpress.org/reference/functions/wp_enqueue_script/) to add JavaScript code from a file.
...
- [wp_register_style()](https://developer.wordpress.org/reference/functions/wp_register_style/) and [wp_enqueue_style()](https://developer.wordpress.org/reference/functions/wp_enqueue_style/) to add CSS from a file.
...
You can read [more information here](../17-internationalization/internationalize-your-plugin.md#text-domains).
...
[Detailed Plugin Guidelines](detailed-plugin-guidelines.md#4-code-must-be-mostly-human-readable)
