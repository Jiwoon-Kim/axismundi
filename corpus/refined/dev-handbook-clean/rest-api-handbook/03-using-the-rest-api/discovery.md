---
source_url: https://developer.wordpress.org/rest-api/using-the-rest-api/discovery/
synced: 2026-05-12
handbook: rest-api
...
```php
// jQuery methodvar $link = jQuery( 'link[rel="https://api.w.org/"]' );var api_root = $link.attr( 'href' ); // Native methodvar links = document.getElementsByTagName( 'link' );var link = Array.prototype.filter.call( links, function ( item ) { return ( item.rel === 'https://api.w.org/' );} );var api_root = link[0].href;
```
...
The `authentication` value is a map (associative array) of authentication method ID to authentication options. The options available here are specific to the authentication method itself. See the [authentication documentation](authentication.md) for the options for specific authentication methods.
...
```json
{ "name": "Example WordPress Site", "namespaces": [ "oembed/1.0/" ]}
```
