---
source_url: https://developer.wordpress.org/rest-api/using-the-rest-api/global-parameters/
synced: 2026-05-12
handbook: rest-api
...
For more about linking and embedding, see the [Linking and Embedding](linking-and-embedding.md) page.
...
```http
HTTP/1.1 302 FoundLocation: http://example.com/wp-json/wp/v2/users/42 { "id": 42, ...}
```

The equivalent enveloped response (with a `GET` to `wp/v2/users/me?_envelope`) would be:

```http
HTTP/1.1 200 OK { "status": 302, "headers": { "Location": "http://example.com/wp-json/wp/v2/users/42" }, "body": { "id": 42 }}
```
...
```html
<script>function receiveData( data ) { // Do something with the data here. // For demonstration purposes, we'll simply log it. console.log( data );}</script><script src="https://example.com/wp-json/?_jsonp=receiveData"></script>
```
