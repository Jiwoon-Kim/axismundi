---
source_url: https://developer.wordpress.org/rest-api/extending-the-rest-api/modifying-responses/
synced: 2026-05-12
handbook: rest-api
...
```php
{ "meta": { "release": { "version": "5.2", "artist": "Jaco" } }}
...
```php
{ "meta": { "release": { "version": "5.2", "artist": "Jaco", "unknown_field": 5.3 } }}
...
```php
{ "meta": { "projects": [ { "name": "WordPress", "website": "https://wordpress.org" }, { "name": "BuddyPress", "website": "https://buddypress.org" } ] }}
...
```json
{ "meta": { "release": [ { "version": "5.2", "artist": "Jaco" }, { "version": "5.1", "artist": "Betty" } ] }}
```
...
```php
<?php$response->add_link( 'author', rest_url( "/wp/v2/users/{$post->post_author}" ), array( 'embeddable' => true,) );$response->add_link( 'author', rest_url( "/wp/v2/users/{$additional_author}" ), array( 'embeddable' => true,) );
```
...
This will convert link URLs from `https://api.mypluginurl.com/my_link` to`my\_plugin:my\_link`in the API response. The full URL must still be used when adding links using`[WP_REST_Response::add_link()](https://developer.wordpress.org/reference/classes/wp_rest_response/add_link/) `.
