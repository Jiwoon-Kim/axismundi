---
source_url: https://developer.wordpress.org/themes/global-settings-and-styles/settings/custom/
synced: 2026-05-12
handbook: theme
chapter: theme-json
...
```json
{ "version": 2, "settings": { "custom": { "fruit": "apple" } }}
```
...
```json
{ "version": 2, "settings": { "custom": { "lineHeight": "1.4em" } }}
```
...
```css
body { --wp--custom--line-height: 1.4em;}
```
...
In the [Styles documentation](../03-styles/index.md), you will learn how to apply styles to the root element, elements, and blocks via `theme.json`. This will be one of the primary use cases for integrating with `settings.custom`.
...
```json
{ "version": 2, "settings": { "custom": { "lineHeight": { "xs": "1", "sm": "1.25", "md": "1.5", "lg": "1.75" } } }, "styles": { "typography": { "lineHeight": "var:custom|line-height|md" } "blocks": { "core/paragraph": { "typography": { "lineHeight": "var:custom|line-height|lg" } } } }}
```
...
Remember, you will learn more about styling via `theme.json` from the [Styles documentation](../03-styles/index.md). You can use what you learn there to combine with the techniques outlined here.
...
```css
.example-class { line-height: var( --wp--custom--line-height--sm );}
```
