---
source_url: https://developer.wordpress.org/themes/templates/template-hierarchy/
synced: 2026-05-12
handbook: theme
chapter: templates
...
As covered in the [Templates](templates.md) documentation, templates are reusable files that are used to generate the document structure of pages on your WordPress site. WordPress’ template loader determines which template file should be loaded on any given URL.
...
- `front-page.html`
- Falls back to the [Home template hierarchy](template-hierarchy.md#home-hierarchy)

If **A static page** is chosen for the **Front page displays** setting, the hierarchy is:

- `front-page.html`
- Falls back to the [Page template hierarchy](template-hierarchy.md#page-hierarchy)
...
- `{mime_type}.html`
- `attachment.html`
- Falls back to the default [Single template hierarchy](template-hierarchy.md#single-hierarchy)
...
The core WordPress `post` post type uses the [Home template hierarchy](template-hierarchy.md#home-hierarchy), and the default `page` and `attachment` post types do not have archive views.
