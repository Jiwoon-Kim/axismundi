---
source_url: https://developer.wordpress.org/themes/patterns/starter-patterns/
synced: 2026-05-12
handbook: theme
chapter: patterns
...
Technically, any pattern in your theme can be converted to a page pattern. All you need to do is define the correct parameters to mark it as such. As described in [Registering Patterns](registering-patterns.md), there are two available file header fields that you must set to make this happen:
...
As noted in [Registering Patterns](registering-patterns.md), there is a single required header field needed for your template pattern (and one optional field):
...
The following list includes the templates that you can define via the `Template Types` field, but you can always reference the [Template Hierarchy](../04-templates/template-hierarchy.md) documentation for a full overview of templates:
...
To learn more about including patterns in templates, check out the [Usage in Templates](usage-in-templates.md) documentation.
