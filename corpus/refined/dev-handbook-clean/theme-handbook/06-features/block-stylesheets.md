---
source_url: https://developer.wordpress.org/themes/features/block-stylesheets/
synced: 2026-05-12
handbook: theme
chapter: features
...
When styling blocks, you should always do so via the [`theme.json` styles property](../03-theme-json/03-styles/index.md) if possible. This ensures that your styles have the best compatibility across the system, working alongside the default WordPress styles, those added by plugins, and user customizations.
...
The primary use case for block stylesheets is when you have too much CSS to add to [`styles.blocks.{blockname}.css`](../03-theme-json/03-styles/styles-reference.md#css) in `theme.json`. This property allows you to add custom CSS, but it’s only ideal when it’s just a small bit of code. This is because you lose out on syntax highlighting and must place everything in a single line (JSON doesn’t support line breaks).
...
```text
.wp-block-super-duper { /* custom CSS goes here. */}
```
