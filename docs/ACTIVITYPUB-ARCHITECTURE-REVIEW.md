...
### 8c.2 wp.org 리뷰 대응 — 0.1.1 (2026-09-23)

제출본이 **pended**로 돌아왔다. 지적 넷이 왔고 넷 다 실물이었다.

| 지적 | 대응 |
|---|---|
| **Phoning home** — Announce 대상을 자동으로 fetch하며 사이트 주소와 IP를 상대에게 노출 | 옵트인 설정 신설(`ax_op_background_acquisition`, **기본 꺼짐**). 예약 시점과 실행 시점 양쪽에서 확인하고, 끄면 남은 예약도 청소한다. Tools › Remote Objects에 스위치와 설명 |
| `echo wp_json_encode( …, JSON_UNESCAPED_SLASHES \| JSON_UNESCAPED_UNICODE )` 3곳 | 플래그 제거. 리뷰어가 "두 번째 인자로 이스케이프를 우회하지 말라"고 짚은 지점 |
| 콜백이 돌려주는 필터 HTML 2곳 (card-body 템플릿, object-view interactions) | 계약을 좁혔다. 템플릿은 `parse_blocks` → **등록된 블록만** 재직렬화, interactions는 HTML 대신 **블록 목록**을 받아 OP가 `render_block` |
| i18n — `__( $definition[0], … )` | 제목을 맵에서 리터럴 gettext로 |

**세 번째는 대응이 아니라 개선이다.** 원시 HTML이 표현 층에 들어올 자리가 사라졌다. 두 필터
모두 이제 "블록을 이름으로 지목"하는 seam이고, 페이지에 나가는 것은 항상 등록된 블록이 만든
것이다. 외부 소비자가 없어 가능한 변경이었다(§8c의 측정).

**privacy 기본값이 계약이 되었다.** `audit-remote-fetch`가 세 가지를 단정한다. 꺼져 있으면 아무
것도 예약하지 않고, 켜면 동기 fan-out 없이 지연 Actor 조회 하나만 예약하며, 꺼진 뒤에는 이미
예약된 작업도 네트워크에 닿지 않는다.

같은 릴리스에 **FEP-044f 같은 작성자 예외**(§8d.1)도 넣었다. 두 작성자가 모두 알려진 경우에만
`self`로 올리고, 대상 작성자를 모르면 그대로 미승인이다. 모르면 추측하지 않는다.

Plugin Check는 여전히 ERROR 0이다. 남은 경고 셋은 `$GLOBALS[$key]` 복원 두 곳과 접두사가 이미
붙은 변수 하나로, 전부 오탐이다.
...
22. ~~수신 인용의 같은 작성자 예외~~ → 0.1.1에서 구현(§8c.2). 두 작성자를 모두 알 때만 `self`.
