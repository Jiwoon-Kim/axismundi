...
function axismundi_act_render_profile_follow_control( string $content ) : string {
	...
	$subject = axismundi_act_current_local_actor();
	if ( ! $target instanceof Axismundi_Actor || ! $subject instanceof Axismundi_Actor || ! $target->is_local() || 'Person' !== $target->get_type() || $target->get_uri() === $subject->get_uri() || 'public' !== $target->get_status() ) {
		return $content;
	}
	$relation = axismundi_act_get_relation( 'follow', $subject->get_uri(), $target->get_uri() );
	$state    = is_array( $relation ) ? (string) $relation['state'] : '';
	...
	<div class="ax-local-follow">
		...
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			...
			<?php wp_nonce_field( 'axismundi_act_local_follow_' . hash( 'sha256', $target->get_uri() ) ); ?>
			<button type="submit" class="wp-element-button"><?php echo esc_html( $label ); ?></button>
		</form>
	</div>
	<?php
...
function axismundi_act_handle_local_follow() : void {
	...
	$result = 'follow' === $intent
		? axismundi_act_follow_local_actor( $subject, $target )
		: axismundi_act_unfollow_local_actor( $subject, $target );
