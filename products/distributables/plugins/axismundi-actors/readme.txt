...
* Adds the generic Dislike control to bundled Person and Group profile card
  templates, where community context resolves the pair as one vote.

= 0.0.66 =
* Gives a Group profile one editable Activity layout with numbered pages and
  Posts/Comments filters, while Person profiles retain their Activity and Community tabs.
...
= 0.0.60 =
* Adds a managed-Group administration section hook so domain plugins can place
  Group-scoped controls beside the Actor profile without creating a duplicate screen.

= 0.0.57 =
* Let the shared editor mention search return a canonical plaintext `@handle` or
  `@handle@domain` token alongside the stable Actor URI, without discovering remote
  accounts while a writer types.
...
* Add editable Account Header, Actor Avatar, Name, Handle, Biography, and Profile Fields blocks for local and remote profile templates.
* Add list and grid profile-link displays with reciprocal-link verification indicators, plus Core-compatible avatar sizing, border, shadow, and profile-link controls.
...
* Compose the public Activity-ledger feed block into the default Actor profile
  template. Activities remains the owner of the feed; Actors keeps the profile
  shell and adds no content archive.
