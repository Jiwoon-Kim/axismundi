...
try {
	...
	ax_remote_admin_assert( $ax_remote_admin_results, 'refreshing a cached Actor reuses its verified acct address when available', $actor instanceof Axismundi_Actor && '@admin_fixture@example.com' === axismundi_actors_remote_refresh_input( $actor ) );
	if ( $actor instanceof Axismundi_Actor ) {
		$wpdb->delete( axismundi_actors_addresses_table(), array( 'identity_id' => $actor->get_identity_id() ), array( '%d' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- fixture setup.
		ax_remote_admin_assert( $ax_remote_admin_results, 'an older cached Actor without an acct refreshes through its profile URL to recover WebFinger verification', 'https://example.com/@admin_fixture' === axismundi_actors_remote_refresh_input( $actor ) );
		axismundi_actors_record_verified_acct_address( $actor->get_identity_id(), 'admin_fixture@example.com' );
	}
