...
function axismundi_cal_rest_envelope( int $post_id ) : array {
	...
	if ( ! is_array( $envelope ) ) {
		...
		return array(
			...
			'joinMode'                 => 'none',
			'joinEligibility'          => 'public',
			'participantVisibility'    => 'organizers',
	...
	return array(
		...
		'joinEligibility'          => (string) ( $envelope['join_eligibility'] ?? 'public' ),
		'participantVisibility'    => (string) ( $envelope['participant_visibility'] ?? 'organizers' ),
...
function axismundi_cal_rest_to_fields( array $value ) : array {
	$map = array(
		...
		'joinMode'                 => 'join_mode',
		'joinEligibility'          => 'join_eligibility',
		...
		array(
			...
			'schema'          => array(
				...
				'context'    => array( 'edit' ),
					...
					'joinEligibility'          => array( 'type' => 'string', 'enum' => AXISMUNDI_CAL_JOIN_ELIGIBILITIES ),
					'participantVisibility'    => array( 'type' => 'string', 'enum' => AXISMUNDI_CAL_PARTICIPANT_VISIBILITIES ),
