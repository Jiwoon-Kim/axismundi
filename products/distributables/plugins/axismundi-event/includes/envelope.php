 ...
 */
function axismundi_event_get( int $post_id ) : ?array {
	global $wpdb;
	if ( $post_id <= 0 || ! axismundi_event_ready() ) {
		return null;
	}
 ...
 */
function axismundi_event_normalize_datetime( string $value, string $timezone ) {
	$value = trim( $value );
	if ( '' === $value ) {
		return new WP_Error( 'ax_event_datetime', __( 'A date and time is required.', 'axismundi-event' ), array( 'status' => 400 ) );
	}
	try {
		$zone  = new DateTimeZone( '' !== $timezone ? $timezone : 'UTC' );
		$local = new DateTimeImmutable( $value, $zone );
	} catch ( Exception $error ) {
		return new WP_Error( 'ax_event_datetime', __( 'The date, time or timezone could not be read.', 'axismundi-event' ), array( 'status' => 400 ) );
	}
	return array(
 ...
 */
function axismundi_event_save( int $post_id, array $fields ) {
	global $wpdb;
	if ( ! axismundi_event_ready() ) {
		return new WP_Error( 'ax_event_store', __( 'The event store is unavailable.', 'axismundi-event' ) );
	}
	$post = get_post( $post_id );
	if ( ! $post instanceof WP_Post || AXISMUNDI_EVENT_POST_TYPE !== $post->post_type ) {
		return new WP_Error( 'ax_event_post', __( 'An Event post is required.', 'axismundi-event' ), array( 'status' => 400 ) );
	}
	...
	$starts = axismundi_event_normalize_datetime( (string) ( $fields['starts_at'] ?? ( $existing['starts_at'] ?? '' ) ), $timezone );
	if ( is_wp_error( $starts ) ) {
		return $starts;
	}
	$ends = axismundi_event_normalize_datetime( (string) ( $fields['ends_at'] ?? ( $existing['ends_at'] ?? '' ) ), $timezone );
	if ( is_wp_error( $ends ) ) {
		return $ends;
	}
	/*
	...
	if ( strtotime( $ends['gmt'] ) <= strtotime( $starts['gmt'] ) ) {
		return new WP_Error( 'ax_event_range', __( 'An Event must end after it starts.', 'axismundi-event' ), array( 'status' => 400 ) );
	}

	$status = (string) ( $fields['event_status'] ?? ( $existing['event_status'] ?? 'EventScheduled' ) );
	if ( ! in_array( $status, axismundi_event_statuses(), true ) ) {
		return new WP_Error( 'ax_event_status', __( 'That event status is not one FEP-8a8e defines.', 'axismundi-event' ), array( 'status' => 400 ) );
	}
	$join_mode = (string) ( $fields['join_mode'] ?? ( $existing['join_mode'] ?? 'free' ) );
	if ( ! in_array( $join_mode, axismundi_event_join_modes(), true ) ) {
		return new WP_Error( 'ax_event_join_mode', __( 'That participation mode is not one FEP-8a8e defines.', 'axismundi-event' ), array( 'status' => 400 ) );
	}
	...
	if ( 'external' === $join_mode && '' === $external ) {
		return new WP_Error( 'ax_event_external_url', __( 'External participation needs the URL people are sent to.', 'axismundi-event' ), array( 'status' => 400 ) );
	}
	...
	if ( false === $wpdb->replace( axismundi_event_table(), $data, $formats ) ) {
		return new WP_Error( 'ax_event_write', __( 'The event could not be saved.', 'axismundi-event' ) );
	}

	axismundi_event_refresh_projection( $post_id );
	return true;
}
 ...
 * @return void
 */
function axismundi_event_refresh_projection( int $post_id ) : void {
 ...
 * @return void
 */
function axismundi_event_sync_projection_on_status( int $post_id ) : void {
	axismundi_event_refresh_projection( $post_id );
}
add_action( 'save_post_' . AXISMUNDI_EVENT_POST_TYPE, 'axismundi_event_sync_projection_on_status', 30, 1 );
add_action( 'trashed_post', 'axismundi_event_sync_projection_on_status', 10, 1 );
add_action( 'untrashed_post', 'axismundi_event_sync_projection_on_status', 10, 1 );
