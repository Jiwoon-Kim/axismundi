 ...
 *
 * Without this, publishing is silently ineffective. `axismundi_event_transformer_supports()`
...
$GLOBALS['axismundi_event_rest_write'] = false;
 ...
 * @return mixed
 */
function axismundi_event_clear_rest_write( $response, $handler, WP_REST_Request $request ) {
	if ( preg_match( '#^/wp/v2/' . preg_quote( AXISMUNDI_EVENT_POST_TYPE, '#' ) . '(?:/|$)#', $request->get_route() ) ) {
		$GLOBALS['axismundi_event_rest_write'] = false;
	}
	return $response;
}
add_filter( 'rest_request_after_callbacks', 'axismundi_event_clear_rest_write', 100, 3 );
 ...
 */
function axismundi_event_guard_rest_publish( $prepared, WP_REST_Request $request ) {
	...
	$GLOBALS['axismundi_event_rest_write'] = true;
	if ( ! is_object( $prepared ) || 'publish' !== (string) ( $prepared->post_status ?? '' ) ) {
		return $prepared;
	}
	$post_id = (int) ( $prepared->ID ?? 0 );
	if ( $post_id > 0 && null !== axismundi_event_get( $post_id ) ) {
		return $prepared;
	}
	$incoming = $request['axismundi_event_envelope'];
	if ( is_array( $incoming ) && axismundi_event_rest_envelope_is_writable( $incoming ) ) {
		return $prepared;
	}
	return new WP_Error(
		'ax_event_incomplete',
		__( 'An Event needs a start, an end and a timezone before it can be published.', 'axismundi-event' ),
		array( 'status' => 400 )
	);
}
add_filter( 'rest_pre_insert_' . AXISMUNDI_EVENT_POST_TYPE, 'axismundi_event_guard_rest_publish', 10, 2 );
 ...
 */
function axismundi_event_guard_publish( array $data, array $postarr ) : array {
	if ( AXISMUNDI_EVENT_POST_TYPE !== ( $data['post_type'] ?? '' ) || 'publish' !== ( $data['post_status'] ?? '' ) ) {
		return $data;
	}
	if ( ! empty( $GLOBALS['axismundi_event_rest_write'] ) ) {
		return $data;
	}
...
add_filter( 'wp_insert_post_data', 'axismundi_event_guard_publish', 10, 2 );
