...
const AXISMUNDI_FORUM_DB_VERSION = '5.6';
...
function axismundi_forum_install() : void {
	...
	dbDelta(
		"CREATE TABLE {$settings} (
			...
			comment_posting_policy varchar(12) NOT NULL DEFAULT 'open',
			membership_policy varchar(12) NOT NULL DEFAULT 'open',
			topic_approval_policy varchar(12) NOT NULL DEFAULT 'open',
			comment_approval_policy varchar(12) NOT NULL DEFAULT 'open',
			distribution_scope varchar(12) NOT NULL DEFAULT 'public',
	...
	if ( ! $legacy
		...
		&& in_array( 'comment_posting_policy', $settings_columns, true )
		&& in_array( 'comment_approval_policy', $settings_columns, true )
 ...
 * @return array{group_identity_id:int,posting_policy:string,comment_posting_policy:string,membership_policy:string,topic_approval_policy:string,comment_approval_policy:string,distribution_scope:string,created_at:string,updated_at:string}|null
 */
function axismundi_forum_get_community( int $group_identity_id ) : ?array {
	...
	if ( null === $row ) {
		return array( 'group_identity_id' => $group_identity_id, 'posting_policy' => 'open', 'comment_posting_policy' => 'open', 'membership_policy' => 'open', 'topic_approval_policy' => 'open', 'comment_approval_policy' => 'open', 'distribution_scope' => 'public', 'created_at' => '', 'updated_at' => '' );
	}
	return array(
		'group_identity_id' => (int) $row['group_identity_id'],
		'posting_policy'    => (string) $row['posting_policy'],
		'comment_posting_policy' => (string) $row['comment_posting_policy'],
		'membership_policy' => (string) $row['membership_policy'],
		'topic_approval_policy' => (string) $row['topic_approval_policy'],
		'comment_approval_policy' => (string) $row['comment_approval_policy'],
		'distribution_scope' => (string) $row['distribution_scope'],
...
function axismundi_forum_update_community_policy( int $group_identity_id, int $user_id, string $column, string $value ) {
	if ( ! in_array( $column, array( 'posting_policy', 'comment_posting_policy', 'membership_policy', 'topic_approval_policy', 'comment_approval_policy', 'distribution_scope' ), true ) || ! axismundi_forum_is_community( $group_identity_id )
	...
	$table = axismundi_forum_settings_table();
	$sql = 'INSERT INTO ' . $table . ' (group_identity_id, posting_policy, comment_posting_policy, membership_policy, topic_approval_policy, comment_approval_policy, distribution_scope, created_at, updated_at) VALUES (%d, %s, %s, %s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE ' . $column . ' = VALUES(' . $column . '), updated_at = VALUES(updated_at)';
	...
	$comment_posted = 'comment_posting_policy' === $column ? $value : (string) $existing['comment_posting_policy'];
	...
	$comment_approval = 'comment_approval_policy' === $column ? $value : (string) $existing['comment_approval_policy'];
	$scope = 'distribution_scope' === $column ? $value : (string) $existing['distribution_scope'];
	$written = $wpdb->query( $wpdb->prepare( $sql, $group_identity_id, $posted, $comment_posted, $membership, $topic_approval, $comment_approval, $scope, $now, $now ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- allowlisted policy column and Group-keyed upsert.
