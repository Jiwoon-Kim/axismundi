...
try {
	...
	ax_auth_assert( $ax_auth_results, 'the Note Federation panel uses a searchable BCP-47 selector with an inherited automatic default', is_string( $panel_script ) && false !== strpos( $panel_script, 'ComboboxControl' ) && false !== strpos( $panel_script, 'languageOptions' ) && false !== strpos( $panel_script, 'Automatic (%1$s: %2$s)' ) && false !== strpos( $panel_script, 'Actor default' ) );
