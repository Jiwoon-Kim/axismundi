...
try {
	...
	$editor_php   = file_get_contents( dirname( __DIR__ ) . '/includes/editor.php' );
	...
	ax_auth_assert( $ax_auth_results, 'the Note editor explicitly requests Object Projections\' shared inline-mention completer instead of assuming another editor loaded it', is_string( $editor_php ) && false !== strpos( $editor_php, "function_exists( 'axismundi_op_enqueue_mention_autocomplete' )" ) && false !== strpos( $editor_php, 'axismundi_op_enqueue_mention_autocomplete();' ) );
	...
	ax_auth_assert( $ax_auth_results, 'the Note Federation panel uses a searchable BCP-47 selector with a WordPress-profile automatic default', is_string( $panel_script ) && false !== strpos( $panel_script, 'ComboboxControl' ) && false !== strpos( $panel_script, 'languageOptions' ) && false !== strpos( $panel_script, 'Automatic (%1$s: %2$s)' ) && false !== strpos( $panel_script, 'WordPress profile language' ) );
