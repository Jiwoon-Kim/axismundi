...
try {
	...
	$mention_uri  = $fixture_actor_uri;
	$mention_name = $fixture_actor instanceof Axismundi_Actor ? axismundi_actors_federated_mention_name( $fixture_actor ) : '';
	...
	ax_article_assert( $ax_article_results, 'mentioned-only Article addresses and tags exactly its explicit recipient without Public', is_array( $direct ) && array( $mention_uri ) === $direct['to'] && array() === $direct['cc'] && array( 'type' => 'Mention', 'name' => $mention_name, 'href' => $mention_uri ) === $direct['tag'][0] && ! axismundi_op_post_article_publicly_readable( get_post( $post_id ) ) );
	delete_post_meta( $post_id, AXISMUNDI_OP_POST_MENTIONS_META );
	wp_update_post( array( 'ID' => $post_id, 'post_content' => '<p>Hello <a class="mention" href="' . esc_url( $mention_uri ) . '">@mentioned</a>.</p>' ) );
	$content_direct = axismundi_op_transform_object( get_post( $post_id ) );
	ax_article_assert( $ax_article_results, 'a Core editor mention anchor drives the same Article audience and fully-qualified Mention tag without duplicate recipient metadata', is_array( $content_direct ) && array( $mention_uri ) === $content_direct['to'] && array() === $content_direct['cc'] && array( 'type' => 'Mention', 'name' => $mention_name, 'href' => $mention_uri ) === $content_direct['tag'][0] );
	update_post_meta( $post_id, AXISMUNDI_OP_POST_VISIBILITY_META, 'public' );
	wp_update_post( array( 'ID' => $post_id, 'post_content' => '<p><a class="mention" href="https://arbitrary.example/not-an-actor">invalid</a></p>' ) );
	$unknown_post     = get_post( $post_id );
	$unknown_strict   = axismundi_op_post_article_audience( $unknown_post );
	$unknown_mention  = axismundi_op_transform_object( $unknown_post );
	ax_article_assert( $ax_article_results, 'an unresolved mention fails strict authoring but a public live projection remains readable, keeps its recipient URI, and omits only the unverifiable tag', is_wp_error( $unknown_strict ) && 'ax_op_post_mention_actor' === $unknown_strict->get_error_code() && is_array( $unknown_mention ) && array( axismundi_act_public_audience_uri() ) === $unknown_mention['to'] && in_array( 'https://arbitrary.example/not-an-actor', $unknown_mention['cc'], true ) && ! isset( $unknown_mention['tag'] ) && axismundi_op_post_article_publicly_readable( $unknown_post ) );
	wp_update_post( array( 'ID' => $post_id, 'post_content' => '<p>Restored fixture content.</p>' ) );
