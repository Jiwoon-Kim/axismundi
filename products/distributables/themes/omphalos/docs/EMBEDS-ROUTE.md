...
- **Centre fixed-width provider embeds**: `iframe { margin-inline: auto }`. Many
  ...
  - Script-hydrated social (X, Bluesky) is a SPECIAL case: the widget hydrates the
    blockquote into a fixed-width CONTAINER that holds the iframe
    (`.twitter-tweet-rendered` ~500, `.bluesky-embed` ~600), so `margin-inline:auto`
    on the iframe only centres within that box. Centre THAT container instead
    (`.wp-block-embed :is(.twitter-tweet-rendered, .bluesky-embed)`) — verified X
    500/645 → 73px each side, Bluesky 600/645 → 22.5px each side.
