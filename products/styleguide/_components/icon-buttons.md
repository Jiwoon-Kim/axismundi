...
**여기까지가 이 페이지가 쓰는 소스에 한한 이야기입니다.** 아이콘 레지스트리나 정적
폰트에는 움직일 축이 없고, 그때는 `Icon(selected)`가 정말로 저장해야 하는 두 번째
이름입니다 — 레지스트리는 아이콘 하나가 SVG 하나라 채워진 별이 **따로 등록된 다른
아이콘**입니다. 세 소스를 한 표로 둔 곳은
[Buttons]({{ '/components/buttons/' | relative_url }})입니다.
...
실측으로 다섯 크기 × 세 폭이 전부 발행 표와 일치합니다.

```
XS   28x32    32x32    40x32
S    32x40    40x40    52x40
M    48x56    56x56    72x56
L    64x96    96x96    128x96
XL   104x136  136x136  184x136
```
...
**target size는 container size가 아닙니다.** 이 구분이 구현을 가릅니다.

처음엔 `min-inline-size: 48px`으로 상자를 넓혔는데, 두 가지가 동시에 틀렸습니다.
스펙은 XS를 32dp 폭으로 그리므로 **상자를 키우는 것 자체가 스펙 위반**이고, 게다가
**이 컴포넌트에만 있는 width 축을 지웠습니다** — XS에서 28·32·40이 전부 48로 뭉개져
narrow와 default와 wide가 같은 그림이 됐습니다.

지금은 상자를 두고 **타깃만 키웁니다.**

```css
.wp-block-axismundi-icon-button::after {
  content: "";
  position: absolute;
...
절대 위치라 자리를 차지하지 않으니 레이아웃이 움직이지 않고, `max()` 덕에 M 이상에서는
상자와 같아져 규칙 하나로 다섯 크기를 덮습니다. 실측하면 **XS narrow는 상자 28×32,
타깃 48×48**입니다.
