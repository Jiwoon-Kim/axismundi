...
axismundi_activitypub_bridge_unregister_mention_presentation();
ax_bridge_assert( $ax_bridge_results, 'the official mention presentation filter yields to Axismundi plaintext mention ownership', false === has_filter( 'the_content', array( 'Activitypub\\Mention', 'the_content' ) ) );
...
ax_bridge_assert( $ax_bridge_results, 'the official QuoteRequest domain handler yields to the Activities-owned consent state machine', false === has_action( 'activitypub_inbox_quote_request', array( 'Activitypub\\Handler\\Quote_Request', 'handle_quote_request' ) ) );
...
ax_bridge_assert( $ax_bridge_results, 'the Bridge owns a verified private delivery table and worker hook', axismundi_activitypub_bridge_delivery_ready() && ! post_type_exists( AXISMUNDI_ACTIVITYPUB_BRIDGE_LEGACY_POST_TYPE ) && 10 === has_action( AXISMUNDI_ACTIVITYPUB_BRIDGE_DELIVERY_HOOK, 'axismundi_activitypub_bridge_process_delivery' ) );
