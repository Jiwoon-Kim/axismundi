 ...
 *
 * An icon block, and the leading icon of a basic dialog header (M3 optional
 * hero icon). Decorative unless it has a Label, as core/icon: with one it is an
 * image with that name, on either source.
 *
 * The painted element comes from the shared renderer, includes/icon.php; this
 * file decides only how the block paints and wraps it.
 *
...
// The icon size token, for either source: the glyph reads it through the icon
// font's utility class, the SVG through style.css. Written only when the
// author chose a size, so a container - a sized button - can supply it.
$axismundi_dialogs_icon_size_css = '';
if ( array_key_exists( 'fontSize', $attributes ) || array_key_exists( 'fontSize', $axismundi_dialogs_typography ) ) {
	...
	// A preset becomes its custom property, as the editor's cssFontSize() does.
	// wp_get_typography_font_size_value() handles a raw value only - it passed
	// `var:preset|font-size|large` through as text, which is not CSS, so a
	// preset size never reached the page.
	if ( is_string( $axismundi_dialogs_icon_size ) && preg_match( '/^var:preset\|font-size\|(.+)$/', $axismundi_dialogs_icon_size, $axismundi_dialogs_preset ) ) {
		$axismundi_dialogs_icon_size = 'var(--wp--preset--font-size--' . _wp_to_kebab_case( $axismundi_dialogs_preset[1] ) . ')';
	} elseif ( function_exists( 'wp_get_typography_font_size_value' ) ) {
		$axismundi_dialogs_icon_size = wp_get_typography_font_size_value( array( 'size' => $axismundi_dialogs_icon_size ) );
	}
	if ( is_string( $axismundi_dialogs_icon_size ) && '' !== $axismundi_dialogs_icon_size ) {
		$axismundi_dialogs_icon_size_css = '--md-icon-size: ' . $axismundi_dialogs_icon_size . ';';
		$axismundi_dialogs_icon_axes[]   = $axismundi_dialogs_icon_size_css;
	}
}
...
// They are computed once, for both sources, and land on the painted element:
// the span the shared renderer draws - the glyph itself, or the box around a
// registry SVG. (core/icon paints its <svg>; the span is the primitive's.)
//
...
// Everything the icon reference says goes to the shared renderer
// (includes/icon.php); this file decides only how the block paints it.
$axismundi_dialogs_icon_ref = array(
	'source'         => $axismundi_dialogs_icon_source,
	'name'           => $axismundi_dialogs_icon_value,
	'class'          => (string) ( $attributes['iconClass'] ?? 'material-symbols-outlined' ),
	'flipHorizontal' => ! empty( $attributes['flipHorizontal'] ),
	'flipVertical'   => ! empty( $attributes['flipVertical'] ),
...
if ( 'registry' === $axismundi_dialogs_icon_source ) {
	...
	$axismundi_dialogs_icon_markup = axismundi_dialogs_get_icon(
		$axismundi_dialogs_icon_ref,
		array(
			// A chosen width sizes the icon, not only its box: the height has to
			// follow the viewBox, or a 48px width drew a 24px icon in a 48x24 box
			// while the editor, which already let the height follow, drew 48x48.
			'class' => trim(
				( $axismundi_dialogs_styles['classnames'] ?? '' ) .
				( ! empty( $attributes['style']['dimensions']['width'] ) ? ' ax-icon--has-width' : '' )
			),
			'style' => trim( $axismundi_dialogs_icon_size_css . ' ' . ( $axismundi_dialogs_styles['css'] ?? '' ) ),
			'label' => (string) ( $attributes['ariaLabel'] ?? '' ),
		)
	);
	...
	// The glyph is the painted element, so the skipped supports land on it,
	// as they land on the SVG for a registry icon. Colour goes through
	// convert_vars_to_classnames, as block-supports/colors.php does: a preset
	// is a class, not an inline var. Border and padding do not, as border.php
	// and spacing.php do not - with the option a spacing preset has no class
	// to become, and would be lost.
	...
	$axismundi_dialogs_icon_markup = axismundi_dialogs_get_icon(
		$axismundi_dialogs_icon_ref,
		array(
			'class' => ( $axismundi_dialogs_color_css['classnames'] ?? '' ) . ' ' . ( $axismundi_dialogs_box_css['classnames'] ?? '' ),
			'style' => $axismundi_dialogs_icon_axes . ' ' . ( $axismundi_dialogs_color_css['css'] ?? '' ) . ( $axismundi_dialogs_box_css['css'] ?? '' ),
			'label' => (string) ( $attributes['ariaLabel'] ?? '' ),
		)
	);
	if ( '' === $axismundi_dialogs_icon_markup ) {
	...
	// Two elements, like the registry path and like core/icon: a block box
	// that carries margin and alignment and takes part in its parent's layout,
	// and the glyph inside it that is painted. The wrapper used to BE the glyph
	// - the provider's icon class forces a 1em inline-flex box - so the block
	// had no box a constrained layout could size: in post content it fell out
	// to the container's left edge instead of lining up with the column, and
	// choosing <div> did not help because the class set the display, not the tag.
	// Hidden whole unless it is labelled, as the registry path is: a labelled
	// glyph is an image with a name, and a hidden wrapper would take it away.
	$axismundi_dialogs_icon_attrs = array( 'class' => 'ax-dialog-icon ax-dialog-icon--font' );
	if ( empty( $attributes['ariaLabel'] ) ) {
		$axismundi_dialogs_icon_attrs['aria-hidden'] = 'true';
	}
	$axismundi_dialogs_icon_attrs['data-ax-dialog-icon'] = 'true';
}

// The wrapper is the block's box on both paths: margin and alignment belong to
// it, and it is what a parent layout sizes. The painted element - the glyph or
// the SVG - sits inside it.
