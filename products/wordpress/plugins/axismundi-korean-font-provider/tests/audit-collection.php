<?php
/**
 * Korean provider: the Font Library collection declares the same range as the CSS (dev-only; dist-excluded).
 *
 * Run: npx wp-env run cli wp eval-file wp-content/plugins/axismundi-korean-font-provider/tests/audit-collection.php
 *
...
if ( ! defined( 'AXISMUNDI_KOREAN_FONT_PROVIDER_UNICODE_RANGE' ) ) {
	require_once dirname( __DIR__ ) . '/axismundi-korean-font-provider.php';
}
...
if ( WP_Font_Library::get_instance()->get_font_collection( 'axismundi-korean-font-provider' ) ) {
	wp_unregister_font_collection( 'axismundi-korean-font-provider' );
}
axismundi_korean_font_provider_collection();
$ax_fp_collection = WP_Font_Library::get_instance()->get_font_collection( 'axismundi-korean-font-provider' );
...
foreach ( array( 'Noto Sans KR', 'Noto Serif KR' ) as $ax_fp_name ) {
	...
	ax_fp_assert( $ax_fp_results, "{$ax_fp_name}: the constant matches the stylesheet", AXISMUNDI_KOREAN_FONT_PROVIDER_UNICODE_RANGE === $ax_fp_css_range );
