 ...
 * Plugin URI:        https://github.com/Jiwoon-Kim/axismundi/tree/main/products/wordpress/plugins/axismundi-map
...
define( 'AXISMUNDI_MAP_VERSION', '0.2.2' );
...
function axismundi_map_render_block( array $attributes ) : void {
	...
	if ( '' !== $sync_key ) {
		...
		// Let get_block_wrapper_attributes() escape the JSON at output time, rather than
		// concatenating a pre-built Interactivity attribute into the markup.
		$wrapper_attributes['data-wp-context'] = wp_json_encode( array( 'mapKey' => $sync_key ) );
	}
