<?php
/**
 * Traditional Chinese provider: the Font Library collection declares the same range as the CSS (dev-only; dist-excluded).
 *
 * Run: npx wp-env run cli wp eval-file wp-content/plugins/axismundi-traditional-chinese-font-provider/tests/audit-collection.php
 *
...
if ( ! defined( 'AXISMUNDI_TRADITIONAL_CHINESE_FONT_PROVIDER_UNICODE_RANGE' ) ) {
	require_once dirname( __DIR__ ) . '/axismundi-traditional-chinese-font-provider.php';
}
...
if ( WP_Font_Library::get_instance()->get_font_collection( 'axismundi-traditional-chinese-font-provider' ) ) {
	wp_unregister_font_collection( 'axismundi-traditional-chinese-font-provider' );
}
axismundi_traditional_chinese_font_provider_collection();
$ax_fp_collection = WP_Font_Library::get_instance()->get_font_collection( 'axismundi-traditional-chinese-font-provider' );
...
foreach ( array( 'Noto Sans TC' ) as $ax_fp_name ) {
	...
	ax_fp_assert( $ax_fp_results, "{$ax_fp_name}: the constant matches the stylesheet", AXISMUNDI_TRADITIONAL_CHINESE_FONT_PROVIDER_UNICODE_RANGE === $ax_fp_css_range );
