...
def color_index() -> tuple[Path, int, int]:
    """Write _data/color.yml: which colour tokens exist, and how the schemes swap.

    Names only. The values stay in the CSS the site already loads, so a swatch
    written as `background: var(--md-sys-color-primary)` shows the live value in
    the live scheme and cannot drift from it. What a page cannot get that way is
    the *list* - which roles exist, in what grouping, and which reference tone
    each maps to in either scheme - and hardcoding that list in a document is
    how it goes stale the first time the theme gains a role.
    """
    ref_src = (TOKEN_OUT / "tokens.ref.css").read_text(encoding=UTF8)
    families: dict[str, list[str]] = {}
    for match in re.finditer(r"--md-ref-palette-([a-z-]+?)-(\d+)\s*:", ref_src):
        families.setdefault(match.group(1), []).append(match.group(2))

    def scheme_map(path: Path, block: str | None) -> dict[str, str]:
        text = path.read_text(encoding=UTF8)
        if block:
            match = re.search(re.escape(block) + r"\s*\{(.*?)\n\}", text, re.S)
            if match is None:
                raise SystemExit(f"{path.name}: no {block} block")
            text = match.group(1)
        return {
            m.group(1): m.group(2)
            for m in re.finditer(
                r"--md-sys-color-([a-z-]+)\s*:\s*var\(--md-ref-palette-([a-z0-9-]+)\)", text
            )
        }

    light = scheme_map(TOKEN_OUT / "tokens.sys.color.light.css", None)
    dark = scheme_map(TOKEN_OUT / "tokens.sys.color.dark.css", ':root[data-theme="dark"]')
    neutral = scheme_map(TOKEN_OUT / "tokens.sys.elevation.css", None)

    # Group headings come from the light file's own comments, in file order, so
    # the document reads in the order the theme declares.
    groups: list[dict] = []
    current: dict | None = None
    for line in (TOKEN_OUT / "tokens.sys.color.light.css").read_text(encoding=UTF8).splitlines():
        heading = re.match(r"\s*/\*\s*(.+?)\s*\*/\s*$", line)
        if heading and not heading.group(1).startswith("="):
            current = {"name": heading.group(1), "roles": []}
            groups.append(current)
            continue
        role = re.match(r"\s*--md-sys-color-([a-z-]+)\s*:", line)
        if role and current is not None:
            name = role.group(1)
            current["roles"].append(
                {"name": name, "light": light.get(name, ""), "dark": dark.get(name, "")}
            )

    if neutral:
        groups.append({
            "name": "Scheme-neutral (declared with elevation)",
            "roles": [
                {"name": name, "light": ref, "dark": ref} for name, ref in neutral.items()
            ],
        })

    role_count = sum(len(g["roles"]) for g in groups)
    missing = [
        r["name"] for g in groups for r in g["roles"] if not r["light"] or not r["dark"]
    ]
    if missing:
        raise SystemExit(
            "these colour roles have no reference mapping in one of the schemes: "
            + ", ".join(missing)
        )

    lines = [
        "# GENERATED by tools/generators/sync_styleguide_assets.py - do not edit.",
        "#",
        "# Names only, read from the theme's own colour CSS. The values are not",
        "# here on purpose: a swatch reads them from the stylesheet the page",
        "# already loads, so it shows the live value in the live scheme.",
        "",
        "ref:",
    ]
    for family, tones in families.items():
        lines.append(f"  - family: {family}")
        lines.append(f"    tones: [{', '.join(tones)}]")
    lines.append("")
    lines.append("sys:")
    for group in groups:
        lines.append(f"  - group: {group['name']!r}")
        lines.append("    roles:")
        for role in group["roles"]:
            lines.append(f"      - name: {role['name']}")
            lines.append(f"        light: {role['light']}")
            lines.append(f"        dark: {role['dark']}")
    out = STYLEGUIDE / "_data/color.yml"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text("\n".join(lines) + "\n", encoding=UTF8, newline="\n")
    return out, len(families), role_count
